...
- `style.v12.css` — all styling
- `script.v12.js` — form logic, 15-min change window, PDF copy, Formspree send
...
4. After every edit to CSS/JS, rename to a new version (`style.v13.css`, `script.v13.js`, …) and update the `<link>`/`<script>` tags — filenames (not query strings) defeat caches that ignore `?v=`. Also bump `BUILD` in the script and `<meta name="build">` in `index.html` together — a mismatch makes the page auto-reload once to a consistent pair.
## Before each client / when terms change
Edit at the top of `script.v12.js`:
...
5. Paste it as `FIREBASE_CONFIG` in the script. Done — nav + order form get sign in/up, email verification, locked autofill, per-client order history.
